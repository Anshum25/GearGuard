...
import { Database, Kanban, Calendar, Zap, Shield, MapPin, Clock, Users } from "lucide-react";

const features = [
  {
    icon: Database,
    title: "Equipment as a Single Source of Truth",
    description: "Track ownership, warranty, location, and responsibility for every asset in your organization. Complete visibility across departments with real-time status updates.",
  ...
  },
  {
    ...
    description: "Drag and drop maintenance requests from New → In Progress → Review → Repaired. Technician avatars and overdue indicators keep everyone accountable.",
  ...
  },
  {
    ...
    description: "Schedule and visualize upcoming maintenance with a beautiful calendar interface. Never miss a preventive check with smart reminders and recurring schedules.",
  ...
  },
  {
    ...
    description: "Auto-fill teams and equipment data based on intelligent rules. Reduce manual coordination by 60% with workflow automation that learns your patterns.",
        ...
        >
          <div className="bg-card rounded-2xl border border-border shadow-xl overflow-hidden">
            ...
            <div className="p-6 bg-gradient-to-br from-muted/50 to-background">
              {index === 0 && <EquipmentPreview />}
              {index === 1 && <KanbanPreview />}
              {index === 2 && <CalendarPreview />}
              {index === 3 && <AutomationPreview />}
            </div>
          </div>
  ...
  <div className="space-y-4">
    ...
    <div className="space-y-3">
      {[
        ...
        <div key={i} className="flex items-center gap-4 p-3 bg-background rounded-lg border border-border">
          ...
          <span className={`text-xs px-2 py-1 rounded-full ${item.status === 'Active' ? 'bg-success/20 text-success' : 'bg-warning/20 text-warning'}`}>
            {item.status}
          </span>
        </div>
      ))}
...
const KanbanPreview = () => (
  <div className="space-y-4">
    ...
    <div className="grid grid-cols-4 gap-3">
      {[
        { title: "New", count: 3, color: "muted" },
        { title: "In Progress", count: 2, color: "accent" },
        { title: "Review", count: 1, color: "warning" },
        { title: "Done", count: 5, color: "success" },
      ].map((col) => (
        <div key={col.title} className="bg-muted/50 rounded-lg p-2">
          <div className="flex items-center justify-between mb-2">
            ...
            <span className={`text-xs font-bold text-${col.color}`}>{col.count}</span>
          </div>
  ...
  <div className="space-y-4">
    <div className="flex items-center justify-between">
      ...
      <div className="flex gap-1">
        <button className="w-6 h-6 rounded bg-muted flex items-center justify-center text-muted-foreground">←</button>
        <button className="w-6 h-6 rounded bg-muted flex items-center justify-center text-muted-foreground">→</button>
      </div>
    </div>
    <div className="grid grid-cols-7 gap-1 text-center">
      ...
      {Array.from({ length: 35 }).map((_, i) => {
        ...
        return (
          <div
            key={i}
            className={`text-xs py-2 rounded ${day > 0 && day <= 31 ? 'text-foreground' : 'text-muted-foreground/30'} ${hasEvent ? 'bg-primary text-primary-foreground font-medium' : ''} ${day === 27 ? 'ring-2 ring-primary' : ''}`}
          >
            {day > 0 && day <= 31 ? day : ''}
          </div>
        );
    ...
    <div className="space-y-3">
      {[
        ...
        <div key={i} className="flex items-start gap-3 p-3 bg-background rounded-lg border border-border">
          ...
          <div className={`text-xs px-2 py-0.5 rounded ${rule.active ? 'bg-success/20 text-success' : 'bg-muted text-muted-foreground'}`}>
      ...
      <div className="container mx-auto px-4 relative z-10">
        ...
        >
          ...
          <h2 className="text-3xl md:text-4xl lg:text-5xl font-heading font-bold text-foreground mb-4">
            ...
            <span className="gradient-text">Manage Maintenance</span>
          </h2>
