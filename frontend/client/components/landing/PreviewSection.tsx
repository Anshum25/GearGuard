...
import { Calendar, Kanban, LayoutGrid, Monitor } from "lucide-react";
            ...
            <div className="grid grid-cols-4 gap-4 flex-1">
                {[
                    ...
                    <div key={stat.label} className="bg-card rounded-xl p-4 border border-border shadow-sm">
                        ...
                        <div
                            className={`text-xs ${stat.trend.startsWith("+")
                                    ? "text-success"
...
function KanbanMockup() {
    const cols = [
        { title: "New", count: 4, badgeBg: "bg-muted-foreground/20", badgeText: "text-muted-foreground" },
        { title: "In Progress", count: 3, badgeBg: "bg-accent/20", badgeText: "text-accent" },
        { title: "Review", count: 2, badgeBg: "bg-warning/20", badgeText: "text-warning" },
        { title: "Done", count: 8, badgeBg: "bg-success/20", badgeText: "text-success" },
    ] as const;
                ...
                {cols.map((col) => (
                    <div key={col.title} className="bg-muted/50 rounded-xl p-3 flex flex-col">
                        <div className="flex items-center justify-between mb-3">
                            ...
                            <span className={`text-xs font-bold ${col.badgeBg} ${col.badgeText} px-2 py-0.5 rounded-full`}>
                                {col.count}
                            </span>
                        </div>
                            ...
                            {Array.from({ length: Math.min(col.count, 3) }).map((_, i) => (
                                <div key={i} className="bg-card rounded-lg p-3 shadow-sm border border-border">
                                    ...
                                    <div className="flex items-center justify-between">
                                        ...
                                        <div className={`text-xs px-2 py-0.5 rounded ${col.badgeBg}`}>
        ...
        <div className="h-full flex flex-col">
            ...
            <div className="flex-1 grid grid-cols-7 gap-1">
                ...
                {Array.from({ length: 35 }).map((_, i) => {
                    ...
                    const eventClass =
                        day % 3 === 0 ? "bg-primary" : day % 3 === 1 ? "bg-accent" : "bg-success";
                    ...
                    return (
                        ...
                        >
                            <div className="text-xs text-right text-muted-foreground mb-1">{day > 0 && day <= 31 ? day : ""}</div>
                            {hasEvent && day > 0 && day <= 31 && (
                                <div className={`text-xs px-1 py-0.5 rounded text-primary-foreground ${eventClass}`}>
...
export default function PreviewSection() {
    ...
    const [activePreview, setActivePreview] = useState<PreviewId>("dashboard");

    return (
        <section id="preview" className="py-24 bg-background relative overflow-hidden">
                ...
                >
                    {previews.map((preview) => (
                        <button
                            ...
                            className={`flex items-center gap-2 px-5 py-3 rounded-xl font-medium transition-all duration-300 ${activePreview === preview.id
